...
async def request_sample_webhook(request: Request):
    ...
    result = sf.insert_sample_request(first_name=first_name, last_name=last_name, email=email, phone=phone, company=company, country_code=project, quantity=quantity, products=[str(pid) for pid in product_ids], general_notes=message)
    print("Salesforce Response:", result)
