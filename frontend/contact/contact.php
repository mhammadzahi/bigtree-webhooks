<?php
/**
 * Description: Validates Turnstile locally, then proxies the request to the Python FastAPI backend.
 */
...
add_action('wp_head', function() {
    ...
    <script>
        var bt_contact_vars = {
            ...
            nonce: "<?php echo wp_create_nonce('bt_contact_proxy_nonce'); ?>"
        };
    </script>
    <?php
});
...
// 2. AJAX Handler
add_action('wp_ajax_nopriv_bt_process_contact_proxy', 'bt_handle_contact_proxy');
add_action('wp_ajax_bt_process_contact_proxy', 'bt_handle_contact_proxy');

function bt_handle_contact_proxy() {
    // A. Security Check
    $nonce = isset($_SERVER['HTTP_X_WP_NONCE']) ? sanitize_text_field($_SERVER['HTTP_X_WP_NONCE']) : '';
    if (!wp_verify_nonce($nonce, 'bt_contact_proxy_nonce')) {
        wp_send_json_error(array('detail' => 'Invalid security token. Please refresh the page.'), 403);
    }

    // B. Parse Incoming JSON Payload
    ...
    if (empty($verify_body['success'])) {
        wp_send_json_error(array('detail' => 'Security check failed. Please refresh the page and try again.'), 403);
    }

    // Remove turnstile response from data before sending to Python
    ...
    $api_endpoint = 'https://app.bigtree-group.com/bt-contact-webhook-v2-1';
    ...
    if ($response_code >= 200 && $response_code < 300) {
        ...
        // If the Python API returns a 422 or 500, pass it straight through
